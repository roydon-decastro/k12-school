                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-4 gap-4">
                        ...
                        <div class="">
                            ...
                            <select class="w-full bg-white  border border-gray-200 rounded-lg "
                                wire:model="selectedSection" wire:change="filterBySection">
                        ...
                        <div>
                            <label for="quarter"
                                class="block mr-4 mt-3 mb-1 text-lg font-bold dark:text-white border-slate-300">Quarter</label>
                            <select class="w-full bg-white  border border-gray-200 rounded-lg " wire:model="quarter"
                                required>
                                ...
                                <div class="flex mt-4">
                                    @foreach ($level_subjects as $index2 => $level_subject)
                                        ...
                                        <input wire:model="student_grades.{{$index}}.grades.{{$index2}}.{{$level_subject->name}}" class="w-20 bg-white  border border-gray-200 rounded-lg " type="number" max="100"> --}}
                                            <label
                                                for="student_grades.{{ $index }}.grades.{{ $level_subject->name }}"
                                                class="block mt-3 mb-1 text-sm font-normal dark:text-white">{{ $level_subject->short_name }}</label>
                                            <input
                                                wire:model="student_grades.{{ $index }}.grades.{{ $level_subject->name }}"
                                                class="w-20 bg-white  border border-gray-200 rounded-lg " type="number"
                                                max="100">
                                        </div>
                                    @endforeach
                                </div>
                                <div class="block mt-4">
                                    <label for="student_grades.{{ $index }}.section_student_id"
                                    ...
                                    <select class="w-36 bg-white  border border-gray-200 rounded-lg "
                                        wire:model="student_grades.{{ $index }}.section_student_id" required>
