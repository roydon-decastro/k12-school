                ...
                @if(!empty($section_students))
                    <div class="mb-4">
                        ...
                        <div class="border border-slate-300 p-4 mt-2">
                            <p wire:model='attendance.{{ $index }}.id' class=" text-lg font-medium">{{ ucwords($section_student->studentName) }}</p>
                            <hr>
                            <div class="flex mt-2 overflow-y-auto p-4">
                                <div class="mr-2">
                                    <label for="attendance.{{ $index }}.absent_count" class="block text-sm font-normal  text-gray-900 dark:text-white">Number of Absences</label>
                                    <input wire:model='attendance.{{ $index }}.absent_count' type="number" step="1" class="text-sm font-normal bg-white border border-gray-200 rounded-lg" required>
                                </div>
                                @if($attendance[$index]['absent_count'] > 0)
                                    @foreach(range(1, $attendance[$index]['absent_count']) as $index2)
                                        <div class="mr-2">
                                            <label for="attendance.{{ $index }}.absent_dates_array.{{ $index2 }}" class="block  text-sm font-normal  text-gray-900 dark:text-white">Absent Date</label>
                                            <input class=" text-sm font-normal bg-white border border-gray-200 rounded-lg" wire:model="attendance.{{ $index }}.absent_dates_array.{{ $index2 }}" type="date" required min="2022-06-01" max="2023-04-30">
                                        </div>
                                    @endforeach
                                @endif
                            </div>
